...
class ProductionSystem {
        ...
        try {
            ...
            for (const item of productNorma.materials) {
                ...
                if (!material || material.quantity < requiredQuantity) {
                    await session.abortTransaction();
                    session.endSession();
                    return response.error(res, `Yetarli ${material?.name || 'material'} yo‘q. Kerak: ${requiredQuantity}, Mavjud: ${material?.quantity || 0}`);
                }
            ...
            await ProductionHistory.create([{
                ...
                electricity: productNorma.cost.electricityPerUnit * quantityToProduce // ✅ hisoblangan va yozilgan
            }], { session });
    ...
    async createBn5Production(req, res) {
        ...
        try {
            ...
            if (!date || !bn3Amount || !wasteAmount || !price) {
                await session.abortTransaction();
                session.endSession();
                return response.error(res, "Majburiy maydonlar yetishmayapti: date, bn3Amount, wasteAmount va price kerak");
            }
            ...
            if (finalBn5 < 0) {
                await session.abortTransaction();
                session.endSession();
                return response.error(res, "Chiqarilgan BN-5 miqdori manfiy bo‘lishi mumkin emas");
            }
            ...
            return response.created(res, `BN-5 ishlab chiqarish muvaffaqiyatli: ${finalBn5} kg`, {
                finalBn5,
                price: unitCost,
            ...
            return response.serverError(res, "BN-5 ishlab chiqarishda xatolik", error.message);
        } finally {
            session.endSession();
        }
    }
    ...
    async productionForSalesBN5(req, res) {
        ...
        try {
            ...
            const stakanItems = packagingData.filter(pkg => pkg.label.includes('Stakan'));
            const qopItems = packagingData.filter(pkg => pkg.label === 'Qop');

            const totalStakanQuantity = stakanItems.reduce((sum, pkg) => sum + (parseFloat(pkg.quantity) || 0), 0);
            const totalQopQuantity = qopItems.reduce((sum, pkg) => sum + (parseFloat(pkg.quantity) || 0), 0);
            ...
            for (const pkg of packagingData) {
                ...
                if (!label || !pkgBn5Amount || !quantity || !unit) {
                    throw new Error(`Noto'g'ri qadoqlash ma'lumotlari: ${JSON.stringify(pkg)}`);
                }
            ...
            return response.created(res,
                `${packagingData.reduce((sum, pkg) => sum + pkg.quantity, 0)} dona tashqi bozor uchun muvaffaqiyatli ishlab chiqarildi`,
                {
            ...
            await session.abortTransaction();
            return response.error(res, "Ishlab chiqarish jarayoni muvaffaqiyatsiz yakunlandi", error.message);
        } finally {
            session.endSession();
        }
    }
