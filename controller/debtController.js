...
class DebtController {
    ...
    try {
      ...
      if (!debtId || !amount || !paymentMethod) {
        return response.error(
          res,
          "Qarz ID, miqdor yoki to‘lov usuli kiritilmadi"
        );
      }
      ...
      if (debt) {
        const updatedDebt = await Debt.repayDebt(
          debtId,
          amount,
          paymentMethod,
          session
  ...
  static async getActiveDebts(req, res) {
    try {
      ...
      const combinedDebts = [...debtModelDebts].sort((a, b) =>
      ...
      return response.success(
        res,
        "Qarzlar muvaffaqiyatli olindi",
        combinedDebts
      );
    } catch (error) {
      return response.serverError(res, "Qarzlarni olishda server xatosi", {
        error: error.message,
      });
    }
  }
  ...
  static async getDebtHistory(req, res) {
    try {
      ...
      return response.success(
        res,
        "Qarzlar tarixi muvaffaqiyatli olindi",
        debts
      );
    } catch (error) {
      return response.error(
        res,
        "Qarzlar tarixini olishda xatolik: " + error.message
      );
    }
  }
}
