...
class MaterialService {
  async handleNewIncome(req, res) {
    ...
    try {
      ...
      if (
        !firmData?._id ||
        !mongoose.isValidObjectId(firmData._id) ||
        ...
        return response.error(
          res,
          "Firma ID noto‘g‘ri yoki materiallar to‘liq kiritilishi kerak"
        );
      }
      ...
      for (const item of materialsList) {
        ...
        const existingMaterial = await Material.findOne({ name: item.name })
          .lean()
          .session(session);
        if (existingMaterial) {
      ...
      if (workerPayments && Array.isArray(workerPayments)) {
        for (const payment of workerPayments) {
          if (!payment.workerId || !payment.payment) {
            ...
            return response.error(
              res,
              "Ishchi to‘lovlari ma'lumotlari to‘liq emas"
            );
          }
        }
      }
      ...
      const finalVatAmount =
        vatAmount || (calculatedTotalWithoutVat * finalVatPercentage) / 100;
      const finalTotalWithVat =
        totalWithVat || calculatedTotalWithoutVat + finalVatAmount;
      const finalTotalWithoutVat = totalWithoutVat || calculatedTotalWithoutVat;
      const finalTotalTransportCost =
        totalTransportCost ||
        materialsList.reduce(
          (sum, item) => sum + (item.transportCostPerUnit || 0) * item.quantity,
          0
        );
      const finalTotalWorkerCost =
        totalWorkerCost ||
        materialsList.reduce(
          (sum, item) => sum + (item.workerCostPerUnit || 0) * item.quantity,
          0
        );
      ...
      if (creditUsed > 0) {
        if (creditUsed > debt.remainingAmount) {
          ...
          return response.error(
            res,
            "Kredit ishlatish summasi qolgan qarzdan oshib ketdi"
          );
        }
        ...
        debt.status =
          debt.remainingAmount === 0 ? "fully_paid" : "partially_paid";
      }
    ...
    try {
      ...
      const transactionResult = await (async () => {
        ...
        if (!incomeId || !debtPayment?.amount || !debtPayment?.paymentMethod) {
          throw new Error(
            "Barcha majburiy maydonlar (incomeId, amount, paymentMethod) to'ldirilishi kerak"
          );
        }
        ...
        if (!validPaymentMethods.includes(debtPayment.paymentMethod)) {
          throw new Error(
            `Noto'g'ri to'lov usuli: ${debtPayment.paymentMethod}`
          );
        }
        ...
        const remainingDebt =
          income.debt?.remainingAmount ??
          (income.totalWithVat || income.totalWithoutVat);
        if (!income.debt || remainingDebt <= 0) {
          throw new Error(
            "Qarz mavjud emas yoki u allaqachon to'liq to'langan"
          );
        }

        if (debtPayment.amount > remainingDebt) {
          throw new Error(
            `To'lov summasi (${debtPayment.amount}) qoldiq qarzdan (${remainingDebt}) oshib ketdi`
          );
        }
        ...
        if (
          !balance ||
          balance[debtPayment.paymentMethod] < debtPayment.amount
        ) {
          throw new Error(
            `${debtPayment.paymentMethod}da yetarli mablag' yo'q`
          );
        }
        ...
        income.debt.status =
          income.debt.remainingAmount === 0 ? "fully_paid" : "partially_paid";
      ...
      return response.success(
        res,
        "Qarz to'lovi muvaffaqiyatli amalga oshirildi",
        transactionResult
      );
    } catch (error) {
